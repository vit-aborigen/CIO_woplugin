...
def power_supply(network, power_plants):
    def find_path(start, end, depth, path = None):
        if not path:
            path = []
        path = path + [start]
        if start == end:
            return path
        if not start in graph or len(path) > depth + 1:
            return False

        shortest_path = None
        for node in graph[start]:
            if node not in path:
                new_path = find_path(node, end, depth, path)
                if new_path:
    ...
    for city in graph:
        ...
        for plant in power_plants:
            path = find_path(city, plant, power_plants[plant])
            if path:
